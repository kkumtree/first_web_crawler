...
driver.get(URL)
last_page = int(re.sub(r'[^0-9]', '', driver.find_element_by_class_name('current_next').get_attribute('href')))
...
for i in range(0, last_page):
  ...
  for j in range(0, item_count):
    ...
    item_query = '#prdcList > div:nth-of-type(' + str(item_index) + ') > div.contents > div.gr_explanation > p:nth-child(1) > span'
    query_section = soup.select(item_query)
    col_section = query_section[0].text.strip()

    # 2. 분류별 제목, 제공업체명, 등록일 추출
    item_query = '#prdcList > div:nth-of-type(' + str(item_index) + ') > div.contents > a > p'
    if col_section == "가공":
      # 제목은 일괄처리
      col_item = "가공서비스 제공업체"
      # 제공업체명
      ...
      # 등록일
      item_query = '#prdcList > div:nth-of-type(' + str(item_index) + ') > div.summary > ul > li:nth-child(3) > span'
      col_reg_date = soup.select(item_query)[0].text.strip()
    else:
      # 제목
      col_item = soup.select(item_query)[0].text.strip()
      # 제공업체명
      item_query = '#prdcList > div:nth-of-type(' + str(item_index) + ') > div.contents > div.gr_explanation > p'
      col_company = getCompany(soup.select(item_query)[0].contents)
      # 등록일
      item_query = '#prdcList > div:nth-of-type(' + str(item_index) + ') > div.summary > ul > li:nth-child(4) > span'
      col_reg_date = soup.select(item_query)[0].text.strip()
    ...
    if col_section == "가공":
      # 단가표
      item_query = '#prdcList > div:nth-child(' + str(item_index) + ') > div.summary > ul > li:nth-child(2) > a'
      ...
      # 단가표
      item_query = '#prdcList > div:nth-child(' + str(item_index) + ') > div.summary > ul > li:nth-child(3) > a'
      if (soup.select(item_query)):
        query_table = soup.select(item_query)[0]['onclick'].split('\'')
        col_table = URL_FILE
        for i in query_table:
      ...
      if (soup.select(item_query)):
        ...
        for i in query_sample:
          if 'FILE' in i:
            ...
            col_sample += '&seqno=2'
            break
        item_query = '#prdcList > div:nth-child(' + str(item_index) + ') > div.summary > ul > li.mt5 > span'
    ...
    # 5. 설명추출
    if col_section != "가공":
      item_query =  '#prdcList > div:nth-of-type(' + str(item_index) + ') > div.contents > div.gr_explanation > p:nth-child(2)'
